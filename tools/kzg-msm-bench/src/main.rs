...
fn main() {
    ...
    println!("KZG prover microbenchmark (BLS12-381, arkworks 0.6)");
...
fn evaluate(coeffs: &[Fr], point: Fr) -> Fr {
    coeffs
        .iter()
        .rev()
        .fold(Fr::zero(), |acc, coefficient| acc * point + coefficient)
}
...
fn divide_by_linear(coeffs: &[Fr], point: Fr, value: Fr) -> Vec<Fr> {
    ...
    for index in (1..degree).rev() {
        quotient[index - 1] = coeffs[index] + point * quotient[index];
    }
...
mod tests {
    ...
    fn synthetic_division_reconstructs_polynomial() {
        ...
        let test_point = Fr::from(17_u64);
        assert_eq!(
            evaluate(&quotient, test_point) * (test_point - point) + value,
            evaluate(&coefficients, test_point)
        );
    }
}
