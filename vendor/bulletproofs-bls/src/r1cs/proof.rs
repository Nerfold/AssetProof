...
impl R1CSProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof, R1CSError> {
        ...
        let T_6 = G1Affine::from_compressed(&read48!())
            .map(G1Projective::from)
            .ok_or(R1CSError::FormatError)?;
        let t_x = Scalar::from_be_bytes(&read32!()).ok_or(R1CSError::FormatError)?;
        let t_x_blinding = Scalar::from_be_bytes(&read32!()).ok_or(R1CSError::FormatError)?;
        let e_blinding = Scalar::from_be_bytes(&read32!()).ok_or(R1CSError::FormatError)?;
