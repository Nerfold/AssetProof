...
impl CudaClientInner {
    ...
    async fn connect_inner(cuda_id: u32) -> Result<UnixStream, CudaClientError> {
        ...
        // Keep the same child server alive while it performs cold CUDA/driver
        // initialization. On some GPU hosts this takes several seconds; the
        // upstream 10 x 100ms window tears the child down before its socket is
        // ready and surfaces as NotFound/ConnectionRefused in the caller.
        for _ in 0..600 {
